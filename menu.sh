...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/bagoesvpn/swip/main/main/allow > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/bagoesvpn/swip/main/main/allow | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/bagoesvpn/swip/main/main/allow | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/bagoesvpn/swip/main/main/allow | grep $MYIP | awk '{print $3}')
fi
clear
echo -e "\e[36m╒════════════════════════════════════════════╕\033[0m"
...
if [[ $(cat /opt/.ver) = $serverV ]] > /dev/null 2>&1; then
    ...
    if [[ $x -eq 69 ]]; then
       wget -q -O /usr/bin/update-script "https://raw.githubusercontent.com/bagoesvpn/swip/main/dll/system/update-script.sh" && chmod +x /usr/bin/update-script
